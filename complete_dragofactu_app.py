#!/usr/bin/env python3
"""
DRAGOFACTU - Complete Professional Business Management System
"""
...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QDateEdit, QHeaderView
)
...
# Simple classes outside the main class
class QuoteDialog(QDialog):
    ...
    def add_item(self):
        ...
        self.items_table.setItem(row, 1, QTableWidgetItem("Descripción ejemplo"))
...
class SimpleDashboard(QWidget):
    ...
    def add_document(self):
        QMessageBox.information(self, "Info", "Use el menú superior para crear documentos")
...
class ProductManagementTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.products_table.setHorizontalHeaderLabels(["Código", "Nombre", "Precio", "Stock Actual", "Stock Mínimo", "Estado"])
...
class DocumentManagementTab(QWidget):
    ...
    def new_document(self):
        ...
        if doc_type == "Presupuesto":
            ...
            QMessageBox.information(self, "Información", f"Creando nuevo {doc_type.lower()}...")
        ...
        try:
            with SessionLocal() as db:
                ...
                for row, doc in enumerate(documents):
                    ...
                    self.docs_table.setItem(row, 2, QTableWidgetItem(f"Cliente {doc.client_id}"))
...
class InventoryManagementTab(QWidget):
    ...
    def setup_ui(self):
        ...
        adjust_btn.clicked.connect(self.quick_adjust_stock)
    ...
    def quick_adjust_stock(self):
        QMessageBox.information(self, "Ajustar Stock", "Seleccione un producto y use el botón 'Ajustar' en la tabla")
    
    def adjust_product_stock(self, product):
        quantity, ok = QMessageBox.getInt(
            self, 
            "Ajustar Stock", 
            f"Cantidad (+ para añadir, - para quitar):",
            0, -99999, 99999, 1
        )
        ...
        if ok and quantity != 0:
            try:
                with SessionLocal() as db:
                    ...
                    if db_product:
                        new_stock = (db_product.current_stock or 0) + quantity
                        db_product.current_stock = max(0, new_stock)
                        db.commit()
                        
                        QMessageBox.information(self, "Éxito", f"Stock ajustado para {product.name}: {quantity:+d} unidades")
                        self.refresh_data()
            except Exception as e:
...
class SimpleMainWindow(QMainWindow):
    ...
    def on_tab_changed(self, index):
        ...
        # Refresh specific tab when it becomes active
...
class SimpleApp(QApplication):
    ...
    def show_login(self):
        ...
        if login_dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            self.current_user = current_user  # Keep the original user object
