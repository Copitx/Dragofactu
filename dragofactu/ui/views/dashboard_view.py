    ...
    def refresh(self):
        ...
        try:
            with SessionLocal() as db:
                ...
                client_count = len(client_service.search_clients(active_only=True))
                supplier_count = len(supplier_service.search_suppliers(active_only=True))
                product_count = len(product_service.search_products(active_only=True))
                low_stock_count = len(product_service.get_low_stock_products())
                
                # Get document statistics
                pending_docs = len(document_service.search_documents(status="draft"))
