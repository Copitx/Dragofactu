        ...
        if file_path:
            try:
                if file_path.endswith('.csv'):
                    ...
                    self.export_json_file(file_path)
                    
                QMessageBox.information(self, "✅ Exportado", f"Datos exportados correctamente:\n{file_path}")
                
            except Exception as e:
                QMessageBox.critical(self, "❌ Error", f"Error exportando datos: {str(e)}")
    
    def export_csv_file(self, file_path):
        """Export data to CSV"""
        try:
            import csv
            with SessionLocal() as db:
                ...
                with open(file_path, 'w', newline='', encoding='utf-8') as f:
                    fieldnames = ['code', 'name', 'email', 'phone', 'address', 'tax_id', 'is_active']
                ...
                QMessageBox.information(self, "✅ CSV Exportado", f"Se exportaron {len(clients)} clientes")
                
        except Exception as e:
            raise Exception(f"Error exportando CSV: {str(e)}")
    
    def export_json_file(self, file_path):
        """Export data to JSON"""
        try:
            import json
            with SessionLocal() as db:
                ...
                QMessageBox.information(self, "✅ JSON Exportado", f"Se exportaron {len(clients)} clientes y {len(products)} productos")
                
        except Exception as e:
            raise Exception(f"Error exportando JSON: {str(e)}")
    
    def change_language(self, language_code):
        """Change application language"""
        try:
            if translator.set_language(language_code):
                lang_name = translator.get_available_languages()[language_code]
                self.statusBar().showMessage(f"🌍 Idioma cambiado a: {lang_name}", 3000)
                
                # Update UI elements with new language
                self.update_ui_language()
            else:
                QMessageBox.warning(self, "❌ Error", "No se pudo cambiar el idioma")
        except Exception as e:
            QMessageBox.critical(self, "❌ Error", f"Error cambiando idioma: {str(e)}")
    ...
    def update_ui_language(self):
        ...
        try:
            ...
            if current_lang == 'es':
                ...
                self.setWindowTitle("🐲 DRAGOFACTU - Professional Business Management")
            elif current_lang == 'de':
                self.setWindowTitle("🐲 DRAGOFACTU - Professionelles Geschäftssystem")
            
            # Update tab names
            if hasattr(self, 'tabs'):
                tabs = self.tabs
                if current_lang == 'es':
                    tabs.setTabText(0, "🏠 Panel Principal")
                    tabs.setTabText(1, "👥 Clientes")
                    tabs.setTabText(2, "📦 Productos")
                    tabs.setTabText(3, "📄 Documentos")
                    tabs.setTabText(4, "📊 Inventario")
                    tabs.setTabText(5, "📓 Diario")
                elif current_lang == 'en':
                    tabs.setTabText(0, "🏠 Dashboard")
                    tabs.setTabText(1, "👥 Clients")
                    tabs.setTabText(2, "📦 Products")
                    tabs.setTabText(3, "📄 Documents")
                    tabs.setTabText(4, "📊 Inventory")
                    tabs.setTabText(5, "📓 Diary")
                elif current_lang == 'de':
                    tabs.setTabText(0, "🏠 Hauptseite")
                    tabs.setTabText(1, "👥 Kunden")
                    tabs.setTabText(2, "📦 Produkte")
                    tabs.setTabText(3, "📄 Dokumente")
                    tabs.setTabText(4, "📊 Inventar")
                    tabs.setTabText(5, "📓 Tagebuch")
